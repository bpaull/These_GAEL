# Intra-personal conflict and self-commitment {#fdj}
...
behavior. <!-- why ths sentence here? it is too general after the details given above, and it does not add anything, as far as I can see-->
...
## Experimental Design and Procedure {#mm2}
...
<!-- well, that is not really an argument is it? If we had in mind soft vs hard the whole time, we should have made 50/50. The real reason, I suppose, is that we were interested in GP-style testing and inference at first. Still, we can defend it by saying that the behavior of soft commit is of relative mor einterest to us, for... reasons. But all in all this is a very weak defense of the 75%/25% rule, because it simply does not follow.-->
<!-- expand here --> 
...
[^212]: For example, Milkman et al. (2013) <!-- transform in proper citation --> study temptation and commitment in a sample of gym users. 
...
<!-- Socio-demo info -->
The other novelty of our study is that our subjects are more representative of the general population of the country where the study was conducted than the standard subjects included in many experiments that have dealt so far with the topic of temptation and commitment (for example, @ariely2002procrastination, @casari2009pre, and @houser2018temptation, rely on a population of students). <!-- I know one should not judge from ONE paper, but I have a hard time citing Ariely in papers where i am a coauthor knowing the fabrication he's been proven of doing recently. But up to you. -->
...
<!-- add here that this questionnaire is the Canadian thing (or what is it?) and that it yields three types of players -- else the results by type below do not mean anything; or else get rid of the "type of players" thing in the results. -->
...
Additionally, subjects had to answer a series of questions meant to collect socio-demographic information and to check whether they had played one of FDJ's games in the last 12 months. <!-- same, avoid duplication, said right below --> Specifically, subjects had to report their gender, age, professional activity, and whether they had bought a lottery ticket, played poker online or made any sport bets within the last twelve months.  
...
The second data set concerns the `r sum(!prep_data$in_baseline)/10` subjects who were exposed to the temptation treatment. 
...
We assume that this difference captures a learning effect, and we choose to restrict our analysis to rounds 2 to 10.
<!-- just for the future paper version of this: is the analysis robust to this exclusion? for the thesis, too late and who cares really, it is justified as a restriction (even if in general dropping exp data is really something I do not want to do) -->

For both datasets, we calculated the average before and after treatment for 
each type of player. 
<!-- define what a "type of player" is. Else, readers do not know at this point what you are talking about. -->
...
tib_mean %>% select(6, 7, 5, 3, 4, 2) %>% 
  t() %>% as.data.frame() -> tmp
row.names(tmp) <-  c("baseline low type", "baseline middle type", 
                      "baseline high type", "temptation low type", 
                      "temptation middle type", "temptation high type")
tmp %>% 
  kable(col.names = c("before treatment", "after treatment", "total"),
        digits = 2, 
...
`r round(mean(prep_data$pumps), 2)`, with respect to the risk neutral level of 32.
...
<!-- Wait, I do not follow. HO is "no effect" and we fail to reject it. Right? Here you say that H0 is "there is an effect" and we fail to reject it; but this is wrong I suppose. Is there a "not" missing? -->
...
Contrary to the theoretical expectations, we observe that `r as.numeric(ask_limit$total[2]) * 100`% of the subjects 
asked for a limit. 
This is consistent with commitment choices observed by @houser2018temptation (28.6% 
...
saturation_tib %>% 
  ...
  mutate(limit_apply = if_else(limit_apply, "limit applied", "limit not applied")) %>% 
  select(-1) %>% t() %>% as.data.frame() -> tmp
row.names(tmp) <- c("Limit exceeded before treatment", 
                    "Limit reached before treatment", 
                    "Limit exceeded after treatment", 
                    "Limit reached after treatment")
tmp %>% 
  kable(digits = 2,
        caption = "Share of subject asking for a limit",
        col.names = c("limit not applied", "limit applied")) %>% # note: the order of the headers is here reversed when compared with the mutate() call in line 565. Can you check that this is correct? Looks like (0 is at the right place) but still dubious
...
The share of the subjects who reached and exceeded the limit they asked for in teh first five rounds is the same 
...
$$
And we refer to this inverse as the degree of constraint of the limit. <!-- you mean "we refer to ITS inverse"? i.e. limit/pups? -->
...
tibble(la = c("limit not apply", "limit apply"), # substitute this with "Limit applied" and "Limit not applied" in a way that does not kill your code, I do not do it to avoid problems. 
...
important. <!-- can we say if it is significant with some test or other? -->
3. When the limit is applied, the saturation reduction is 
more important. <!-- again, significant? I know you do MCMC later on, but maybe a little test? -->
...
The differences in mean between the 4 periods before the treatment and the 5 after are 
presented in Figure \@ref(fig:mean-effect2). 
...
Finally, we built a counterfactual: we studied what would have happened to the subjects having 
requested a limit but to whom it was not applied, in case it was applied to them.

```{r mean-effect2, fig.cap='effect on mean for the different groups'}
tibble(n = seq(-3, 1.5, 0.01),
       baseline = bla_blp$ecdf$delta(n),
       `limit refused` = lra_lrp$ecdf$delta(n),
       `limit not applied` = lna_lnp$ecdf$delta(n),
       `limit applied` = laa_lap$ecdf$delta(n),
       `limit applied ex-post` = lea_lep$ecdf$delta(n)) -> treatment_effect

treatment_effect %>% 
  pivot_longer(-n, values_to = "probability", names_to = "treatment") %>% 
  ggplot(aes(n, probability, color = treatment)) + 
  geom_line() +
  geom_vline(xintercet = 0, linetype = "dashed", color = "red", alpha = 0.5)+ #this is to give a reference for the reader
  labs(title = "Effect of treatment on mean pumps")
# can you add conf int around the ECDF or is it too messy / impossible/ too much of a hassle? 
```
...
So for baseline, as expected, there is no significant difference between the four first 
rounds and the subsequent rounds.
3. *limit not applied*: mean of `r round(nearest_n("limit not apply", 0.5), 2)` 
...
risk taken by the subjects. <!-- can we start building a link with the intro and say that this is a soft commitmet? -->
Although this reduction is small, it is significantly different from what is 
observed for the baseline.
4. *limit applied*: mean of `r round(nearest_n("limit apply", 0.5), 2)` 
...
`r round(100 * ct["La", "Ln"], 2)`%. <!-- again, can we link this with hrd commitments so to link results with the intro? -->
5. *limit applied ex-post*: mean of `r round(nearest_n("limit apply ex-post", 0.5), 2)` 
...
This shows that subjects are able to self-constrain. <!-- drop a mention to soft constraints here! -->
...
subjects is applied. <!-- again, refer to soft & hard even in passing here, to connect with the intro -->
...
this hypothesis will have to be tested by future work. <!-- not for the thesis, but this point will have to be explored further for the paper if it will ever be submitted -- which I think it should. -->
...
We see nonetheless that the decrease in risk-taking is more important in subjects for whom 
...
this article. <!-- how? expand at least a little bit -->
...
the constraint by subject. <!-- how? expand at least a little bit -->
