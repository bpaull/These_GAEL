...
[^201]: See Deborah M. Rankin, “How to get ready for retirement: Save, save, 
...
setting out to the Syrens [@elster2000ulysses]. 
...
population [@ashraf2006tying; @milkman2014holding], 
...
[^212]: For example, @milkman2014holding study temptation and commitment in a sample of gym users. 
...
observations for the `r sum(prep_data$in_baseline)/10` subjects who were not 
...
The second data set concerns the `r sum(!prep_data$in_baseline)/10` subjects 
...
tmp %>% 
  kable(digits = 2,
        ...
        col.names = c("limit not applied", "limit applied")) %>% 
...
tibble(la = c("Limit not applied", "Limit applied"),
...
3. *limit not applied*: mean of `r round(nearest_n("limit not applied", 0.5), 2)` 
with credible interval at 95% `r p_ci("limit not applied", 0.475)`.
...
4. *limit applied*: mean of `r round(nearest_n("limit applied", 0.5), 2)` 
with credible interval at 95% `r p_ci("limit applied", 0.475)`.
...
5. *limit applied ex-post*: mean of `r round(nearest_n("limit applied ex-post", 0.5), 2)` 
with credible interval at 95% `r p_ci("limit applied ex-post", 0.475)`).
...
this article.
In @gul2001temptation's model, subjects should choose their constraint in such a
...
This attitude fits better with @noor2015menu's model where self-control costs 
