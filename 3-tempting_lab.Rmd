...
the value of menus. 
Therefore we do not have a mechanism that would allow us to identify the  <!-- why "therefore"? it means "as a consequence of" and I do not see how this consideration follows from the one above... -->
...
of these models. <!-- Our experiment is a pure test of the descriptive validity of the G-P model as compared with EUT and some simple statistical models as a benchmark. [Ok, maybe redundant] -->
...
consequence. <!-- over a set of menus. -->
...
<!-- if you wanted to make an indented list for the incentivized phase, it did not work, and the numbers continue monotonically over this "sub-list". -->
  3. The subject has to bid on 10 pairs of menus by indicating with a slider the 
  ...
  are different.
  4. One of the bids that has just been made is drawn and the subject chooses one 
  of the lotteries that it contains.
  5. A screen indicates to the subject what amount he has won with the steps 4 and 
...
lottery from each of the five menus assigned to him according to her bid and the elicitation mechanism.

<!-- him, her... I know it's tricky but this subject changes gender within three words. maybe keep him? but in general who cares-->
...
an average time of `r round(mean(amt_time/60))` seconds spent on our experiment.
...
This comparisons is used to estimate preferences over singletons, i.e. over individual lotteries.
...
This is a  dummy model, but it is useful as a lower-bar reference point. It seems reasonable to assume that a model incorporating preferences has to be more efficient than this constant response model.
...
This is the model that corresponds to the standard theory in economics where <!-- yu told me you do not like italics and bold and then you use them, so I am forcing you to be consistent :-) -->
...
subtracting the value of the singleton {2}, i.e., a 50\% chance of earning 2 euros. <!-- I thought it better to provide a little reminder here, feel free to delete it if you find it useless -->
...
Since singletons are lotteries, we can test if subjects have <!-- I mean, they *are* lotteries, right? -->
...
`r round(mean(apply(u_estim, 2, valid_pref)* 100), 2)`% have consistent, single-peaked
preferences. 
...
```
<!-- the bowplot here is fine, but it hides the main message that is the position of the coefficients around the zero line. 
It is hard to see since there is variance and your Y axis scale spans nearly 9 units; so the distance from zero becomes too small to be actually noticed. I know that "zooming in" on the data hides ourliers, and only putting means and error bars loses info... but still, if there was a way to have a better take on the values arund zero it would be better. Raincloud? jittered points? beeswarm? up to you.  -->
...
coefficients and bias the interpretation. <!-- I gave my best shot at rewriting this as it made no sense, but I might have gotten it wrong, check -->
...
opposite of these predictions. <!-- is this also a thing that is currently done in ML? if so, say so -->
...
<!-- proposed in @gul2001temptation. -->
<!-- it was a repetition of what is said in better terms below -->
...
better descriptors of our data than economic models. 
