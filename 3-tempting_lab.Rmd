...
@kuhn2014self) or more surprising attitude like in 
...
No selection criteria are applied a priori to the subjects. But only the subjects having filled in a valid end-of-experience code were included in the analysis. Conditions of minimum duration of working time and number of trials are also applied but do not exclude any subject [^321].

[^321]: Only subjects with a valid code are kept because this code allows us to pay the subjects and the author refuses to include subjects who were not paid in the study. The exhaustive inclusion criteria are listed in the pre-registration of the study available with asPredicted code 
...
of a menu. <!-- Instructions are provided in appendix X -> please DO provide instructions, for all experiments; it is standard practice. -->
...
6. The subject is asked to complete a second quiz similar to the one in step 2 
but with different questions. 
7. Main taks: the subject bids on 35 comparisons between 2 menus.
8. 5 of these comparisons are drawn at random and the subject can choose 1 
...
So we ask subject for the maximum amount he is willing to pay to *exchange* one 
...
by the subjects to the one that seemed the most interesting for us[^322]. 
...
The table below summarizes the percentage of subjects for whom we have no 
estimate by item.
...
The graph below shows the distribution of the estimates for each of the items.
...
#### Statistical models estimations
\
...
Now that we have estimated the different parameters for our 5 models, we can compare their performances using 3 different metrics:
...
In the table below, we present the results of the five models according to 
the three metrics, calculated on all the comparisons.


```{r 3_global_models_table}
all_d %>% 
...
The table below shows the performance of each model according to the type of 
comparison. 
...
In the graph below we show that these models have in common to predict that 
subjects are indifferent between two menus much more frequently than what we 
observe.
And that this is also their main difference with the linear regression model 
which has better performances.

```{r 3_models_pred_plot}
all_d %>% 
...
If the economic models often predict the value 0 -- i.e., indifference between the two compared menus -- this is because they evaluate the 
