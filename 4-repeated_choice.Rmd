...
stat_des_df %>%
  t() %>%
  as.data.frame() -> stat_des_df
rownames(stat_des_df) <- c("mean", "sd", "mean r",
                      "sd r", "width of r C.I.",
                      "% estim not in [0,64]")
stat_des_df %>% 
  kable(digits = 2, 
...
as shown in the corresponding column of table \@ref(tab:sum-stat-table4) [^431].
...
tibble(nb_choice = 1:100,
  ...
  labs(title = "Ghost treatments",
       ...
       y = "of case with statistically significant difference",
...
prediction %>% 
  ...
  labs(title = "Prediction models error", 
       x = "Number of training periods",
       y = "MSE") +
  scale_color_viridis_d()
```
