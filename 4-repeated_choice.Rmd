...
always make the same choice? This question has few applied <!-- concrete = béton -->
...
stability of preferences has not been widely treated in this framework, and <!-- on ne paut pas dire que cela n'a jamais été traité, donc on qualifie un peu -->
...
`r round(mean(subjects_ds$is_economist) * 100, 2)`% of our subjects. 
We show that only for
`r round(mean(subjects_ds$is_normal) * 100, 2)`%  of them the answers are
...
measures of risk preferences over our data can lead to the wrong conclusion that a
...
the subjects -- i.e. the serial correlation of answers. <!-- right? or else, correct, but I would add a qualifier here for clarity -->
<!-- and: is it so surprising that subjects exhibit serial correlation? but this is maybe stuff for discussion, see below -->
...
terms is much higher than the average payoffs used in experimental
...
information, form and time. <!-- what do you mean by form here? -->
...
3.  The instructions of the experiment are presented to them. The instructions
    ...
    Instructions are provided in appendix [Appendix C](#expe-instruc4).
...
Analog Risk Task (BART, @lejuez2002evaluation) and the Bomb Risk Elicitation 
...
the learning period. 
We define as *irrationality index* the sum of the
...
```{r spag-plot4, fig.width= 10, fig.height=12, fig.cap='All choice. Each line show the 100 choices of a subject'}
# n_x <- 5
# n_y <- 5
...
<!-- about the plot: it is unclear what the headers in the facets mean. I get that the second header, the one in [...], is an interal for the means of the subjects. It would be better to say so in the text or somewhere. The first header (1 to 5) is never explained, and I am sure they are 5 cuts of the variance, but if you did put the limits for the mean you should put it in the vairance too; how about adding "mean: [interval]" to the second header and "s.d.: [interval]" for the first; then if possible put mean before sd? -->
...
subjects_ds %>% 
  ...
  mutate(obs_0 = pnorm(0, mean_n, sd_n) * 100,
         ...
         range_r = hb_r - lb_r) %>% 
  select(mean_n, sd_n, r_local, sd_r_local, range_r, total_obs) %>% 
...
stat_des_df %>%
  t() %>%
  as.data.frame() -> stat_des_df
rownames(stat_des_df) <- c("mean choice", "sd", "mean r",
                      "sd r", "width of r C.I.",
                      "% estim not in [0,64]")
stat_des_df %>% 
  kable(digits = 2, 
        col.names = c("quant 5%", "quant 25%", "median", 
                      "quant 75%", "quant 95%"),
        caption = "Statistics of individual choice") %>% 
...
`r round(stat_des_df[2, 3], 2)` the variability of the data is
...
as shown in the corresponding column of table <!-- reference to the table missing --> [^431].
...
`r round(stat_des_df[6, 3], 2)`% of the data which would not be
...
`r round(stat_des_df[6, 3], 2)`% of humans is less than 0 cm or greater 
...
```{r norm-test4}
normality_ds %>%
  select(-c(signif_ks_ad, signif_ks_sw, signif_sw_ad)) %>%
  kable(digits = 2,
        col.names = c("KS", "SW", "AD",
                      # "KS & SW", "KS & AD", "SW & AD",
                      "KS & SW & AD", "KS or SW or AD"),
        caption = "Share of subject whose choices pass normality test",
        booktabs = TRUE) 
...
is between `r round(100 - normality_ds$signif_one_pass, 2)`% and
`r round(100 - normality_ds$signif_ks_sw_ad, 2)`% but it is difficult to <!-- just inverted to get an increasing progression -->
give an exact value.
...
used to test hypotheses in experimental economics. <!-- give here the punchline of the argument, else it is hard to follow you through the rest. For instance: "The nature of our data is such that inference from the data is very likely to result in false positive results when using standard techniques in Experimental Economics". We distinguish three different approaches to identify treatment effects: between-, within-subjects, and difference in difference." and then you explain. So readers know what to expect, because right now this thing comes off as a bit abrupt and not easy to follow. -->
...
```{r gt-plot4, fig.cap="Bootstrap t.test at 5% levels for different design"}
...
tibble(nb_choice = 1:100,
  ...
  labs(title = "Ghost treatments",
       subtitle = "100 choices for between treatment and 50 for other treatment",
       x = "% of choices considered per subjects",
       y = "% of significant ghost treatments",
       color = "Type of treatment",
       linetype = "Number of subjects") +
  scale_color_viridis_d()
# consider adding a reference horizontal line at 5%, it will help to see clearly that diffdiff does the job nicely
# achtung: tu dis "%" mais sur les y tu as mis une échelle 0-1, donc ou bien tu mets "share" ou bien (conseillé) tu passess à 0-100
# I changed the labels of the axis, you might disagree, but I find them better
...
<!-- changed a bit the order for more logical flow -->
...
This is particularly striking for the within-subjects case, as no or little individual variation is usually assumed in the 
experimental literature.
The high rate of significant ghost treatments seems to be little
...
```{r corr-plot4, fig.cap="Pearson corelation between subject choice."}
...
<!-- well, that's in theory theory, because it is the same person and it is a bit of a leap of faith to assume iid within the same person... no? to be discussed below -->
...
```{r plt-error-model4, fig.cap="Descriptives power of different models."}
...
```{r dummy-estim4, fig.cap="Predictives power of different models."}
...
prediction %>% 
  ...
  labs(title = "Evolution of the different estimators", #why not use "model error..." as in the title of the previous figure?
       x = "Number of training periods",
       y = "MSE") +
  scale_color_viridis_d()
```
...
<!-- again, is this so surprising that the choices are not independent? -->

This shows that to predict individual behavior, models based on a risk
aversion parameter perform poorly on repetitions of one of the simplest possible task. That <!-- you'd have toa rgue why it is THE simplest so better to tone down -->
...
roughtly `r round(3/0.7*100)` , assuming $x^r$ CRRA.
...
<!-- well, wait, the average model is also not an econ model. econ models (r) are consistently outclassed by regression -->
...
here.
<!-- very good overall. Bravo. But: maybe say a bit more here on this thing I've been hinting at all along -- is it so surprising that observations are correlated? Myabe some considerations on this might be in order (but I realize we are 2 days from the deadline; take it as a consideration/question that will possibly be asked by the rapporteurs and/or come up during the defense. -->
